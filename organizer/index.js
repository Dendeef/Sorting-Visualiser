    ...
    for (let j = 0; j < array.length - i - 1; j++) {
      if (array[j] > array[j + 1]) {
        for (let k = 0; k < bars.length; k++) {
          if (k !== j && k !== j + 1) {
            bars[k].style.backgroundColor = "#90AE7F";
          }
        }
        let temp = array[j];
        array[j] = array[j + 1];
...
async function partition(items, left, right) {
  ...
  bars[pivotIndex].style.backgroundColor = "#90AE7";

  for (let i = 0; i < bars.length; i++) {
    if (i != pivotIndex) {
      bars[i].style.backgroundColor = "#90AE7F";
    }
  }
...
async function InsertionSort(array) {
  ...
  for (let i = 1; i < array.length; i++) {
    ...
    while (j >= 0 && array[j] > key) {
      array[j + 1] = array[j];
      bars[j + 1].style.height = array[j + 1] * heightFactor + "px";
      bars[j + 1].style.backgroundColor = "#90AE7F";
      //bars[j + 1].innerText = array[j + 1];
      await sleep(speedFactor);
...
async function swap(array, i, j, bars) {
  ...
  bars[j].style.height = array[j] * heightFactor + "px";
  bars[i].style.backgroundColor = "#90AE7";
  bars[j].style.backgroundColor = "#90AE7";
  await sleep(speedFactor);

  for (let k = 0; k < bars.length; k++) {
    if (k != i && k != j) {
      bars[k].style.backgroundColor = "#90AE7";
    }
  }
...
async function mergeSort(arr) {
  ...
  while (i < left.length && j < right.length) {
    if (left[i] < right[j]) {
      ...
      // bars[k + middle].style.backgroundColor = "#90AE7";
      // bars[k].innerText = arr[k];
      //await sleep(speedFactor);
    }
    //shift to right side
    ...
    if (k + arr.length < bars.length) {
      ...
      bars[k + arr.length].style.backgroundColor = "#90AE7";
    }
    await sleep(speedFactor);
    //bars[k].innerText = arr[k];
